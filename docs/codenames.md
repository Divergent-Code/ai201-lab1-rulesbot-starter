...
- Must be a single word. Compound words and hyphenated words are not allowed as clues.
- Cannot be any form of a word already visible on the grid.
- Cannot be a codename currently visible on the grid.
- Must relate to the meaning of the words, not their letters or position on the grid.
- Cannot be a proper noun unless the codenames use proper nouns regularly (house rule).
...
- If it is an innocent bystander: cover it with a bystander card. Your turn ends immediately.
- If it is the assassin: cover it with the assassin card. Your team loses immediately.
...
In Codenames: Duet, both players are simultaneously Spymasters and Field Operatives for a cooperative experience against the clock. Each player sees some of the same agents and some different ones on their side of the key card, and both work together to identify 15 agents before the timer runs out.
